...
namespace Server.Commands
{
    public class RegionBounds
    {
        ...
        [Usage("RegionBounds")]
        [Description(
            "Displays the bounding area of either a targeted Mobile's region or the Bounding area of a targeted RegionControl."
        )]
        ...
        private class RegionBoundTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is Mobile)
                {
                    ...
                    {
                        from.SendMessage(
                            "The Region is the Default region for the entire map and as such, cannot have it's bounds displayed."
                        );
                        return;
                    }
                    ...
                    from.SendMessage(
                        String.Format(
                            "That Mobile's region is of type {0}, with a priority of {1}.",
                            r.GetType().FullName,
                            r.Priority.ToString()
                        )
                    );
